      ...
      with(as.list(parbound),
           {
             ...
             ri = rep(exp(runif(n, log(start_g[1]), log(start_g[2]))), length(startbt))
             
             
             #初期環境収容力の範囲から離散型一様分布としてランダムに値をn(デフォルトは10000)個取り出す作業を10回繰り返す
             #ki = rep(exp(runif(n, log(start_k[1]), log(start_k[2]))), length(startbt))  ## get N
             #4/2kei:上記のコードは範囲がstart_gに固定されている。r,kの更新範囲はparboudに格納されるため、本来はparbound$gを範囲にする必要がある。
             ki = rep(exp(runif(n, log(start_k[1]), log(start_k[2]))), length(startbt))
...
### ここからコードスタート　以前はファンクションメイキングパート
        ...
        length(LastCatchYear) > 0 & length(ct) > 1)
    {
      ...
      if (length(g1) >= 10) {
        ...
        #plot(Summary3$graph) #danさんのコードの場合、r,kの範囲更新が正常に行われないため、このグラフの意味がない
        ...
        if(Display == T & NumCPUs == 1 & length(g) > 10) #g-kの候補が11個以上の時、g-kの候補による結果をプロットする
          ...
          plot(
            ...
            xlab = "g",
            ylab = "k (MT)",
            main = "Scatter plot (r-k prior range)",
            sub = paste("Scientific name:",unique(dat$SciName),sep = " ")
          )
          ...
          plot(log(g), log(k), xlab = "ln(g)", ylab = "ln(k)",main="Logarithmic scatter plot (r-k new range)")
          abline(v = mean(log(g)))
          abline(h = mean(log(k)))
