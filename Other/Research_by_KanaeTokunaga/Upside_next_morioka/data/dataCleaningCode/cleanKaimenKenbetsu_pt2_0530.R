...
dt <- read.csv("cleanedData/dataKaimenKenbetsu.csv", encoding = "cp932")
unique(dt$variable)

# Species Name
setwd("~/Dropbox/EDFUPSIDE2018/Data Collection/Cleaning")
species <- read.csv("miscData/fishName.csv")
...
# stock code
stock.list <- read.csv("cleanedData/stock_code_0530.csv")
...
# Read stock data
dt.stock <- read.csv("cleanedData/multiStock_complete_0530.csv", encoding = "cp932")
...
pref.alph <- read.csv("miscData/japan-prefectures-roman-hepburn.csv", header = FALSE,encoding = "cp932")
...
#write.csv(dt.comp, file = "cleanedData/dataProductionByPrefecture_0530.csv")
...
#write.csv(dt.prod.stock, file = "cleanedData/dataProductionByStock_0530.csv")

# Production value data (value data is only available after 2003)
dt.val <- read.csv("cleanedData/dataKaimenSanshutsugaku.csv")
...
#write.csv(dt.comp2, file = "cleanedData/dataProductionValueByPrefecture_0530.csv")
...
#write.csv(dt.prod.value, file = "cleanedData/dataProductionValueByStock_0530.csv")

#Check
#ggplot(subset(dt.prod.value, dt.prod.value$EngName == "Bigeye tuna"), aes(x=year, y = production_vol_t)) +
#        geom_line()
