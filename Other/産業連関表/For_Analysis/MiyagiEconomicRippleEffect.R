...
MiyagiEconomicRippleEffect <- function(Demand_million_yen,TaisyoSangyo){
...
#生産者価格表
#SeisansyaKakaku <- read_csv("SeisansyaKakaku_Miyagi2013.csv")
SeisansyaKakaku <- read_csv("SeisansyaKakaku_Miyagi2015.csv")

#投入係数表
#TounyuKeisu <- read_csv("TounyuKeisu_Miyagi2013.csv")
TounyuKeisu <- read_csv("TounyuKeisu_Miyagi2015.csv")

#逆行列係数表([I-(I-M)A]^-1型を使用する)
#GyakuGyouretsu <- read_csv("Gyakugyouretsu_Miyagi2013.csv")
GyakuGyouretsu <- read_csv("Gyakugyouretsu_Miyagi2015.csv")

#雇用表
#就業係数と雇用係数には"総合"と"個別"がある。
#"総合"は増減した需要によって誘発される就業者・雇用者を表すのに対し、"個別"は当該産業の増減した生産に必要となる就業者・雇用者数を表す。
#つまり、当該産業のみしか考慮されないのである。経済波及効果の文脈ならば総合効果の方が適している。
#しかし、岩手県は個別しかないため、自ら算出する必要がある。宮城県の場合は総合の方の値を用いている。
#Koyou <- read_csv("Koyou_Miyagi2013.csv")
Koyou <- read_csv("Koyou_Miyagi2015.csv")
...
#SeisansyaKakaku2 <- select(SeisansyaKakaku2, table$部門名)
TounyuKeisu2 <- filter(TounyuKeisu, TounyuKeisu$部門名 %in% table$部門名)
TounyuKeisu2 <- select(TounyuKeisu2, table$部門名)
GyakuGyouretsu2 <- filter(GyakuGyouretsu, GyakuGyouretsu$部門名 %in% table$部門名)
GyakuGyouretsu2 <- select(GyakuGyouretsu2, table$部門名)

#対象産業と需要増加額の設定
#TaisyoSangyo <- "水産食料品"
ZoukaGaku <- Demand_million_yen#万円 #2015年は223800百万円(県のサイト:https://www.pref.miyagi.jp/uploaded/attachment/787244.pdfより)
SyouhiTenkan <- 0.684 #2015年の消費転換係数を入れてある
# 消費転換係数
# R1 0.624,  H30 0.661, H29 0.703, H28 0.715, H27 0.684, H26 0.656, H25 0.670
...
table$就業者誘発量 <- round(((table$県内需要増加額 + table$第一次生産誘発額) * Syugyousya))

#雇用表から雇用係数(人/100万円)を抜き出す(末行の合計を除く)
KoyouKeisu <- Koyou[,"雇用係数"][1:nrow(Koyou)-1,]
table$雇用者誘発量 <- round(((table$県内需要増加額 + table$第一次生産誘発額) * KoyouKeisu))
...
MinkanSyouhi <- SeisansyaKakaku2$民間消費支出/sum(SeisansyaKakaku2$民間消費支出,na.rm=T)
...
#雇用表から就業係数(人/100万円)を抜き出す(末行の合計を除く)
table$第二次就業者誘発量 <- round(table$第二次生産誘発額 * Syugyousya)

#雇用表から雇用係数(人/100万円)を抜き出す(末行の合計を除く)
table$第二次雇用者誘発量 <- round(table$第二次生産誘発額 * KoyouKeisu)
