...
file.sources_r = list.files(path = "/Volumes/GoogleDrive/共有ドライブ/gakuLab_Research_Upside/Upside-U.Iwate/Other/GUM-master-Kei/R_Kei",pattern = ".R")
...
file.sources_rda = list.files(path = "/Volumes/GoogleDrive/共有ドライブ/gakuLab_Research_Upside/Upside-U.Iwate/Other/GUM-master-Kei/R_Kei",pattern=".rda")
...
file.sources_rda2 = list.files(path = "/Volumes/GoogleDrive/共有ドライブ/gakuLab_Research_Upside/Upside-U.Iwate/Other/GUM-master-Kei/data",pattern=".rda")
...
setwd("/Volumes/GoogleDrive/共有ドライブ/gakuLab_Research_Upside/Upside-U.Iwate/Other/GUM-master-Kei/R_Kei")
...
#Create_Modelで作成されるregs_JPをregsにすることで変更が少なく済む
regs <- regs_JP
...
fish <- navi %>%
  mutate(Year = as.numeric(Year),Landings_kg = floor(Landings_kg/1000)) %>% #単位をトンにして、切り捨て
  group_by(Fish_Species,Year) %>%
  summarise(Catch = sum(Landings_kg))

fish <- fish %>%
  left_join(lifehistory,by=c("Fish_Species"="CommName"))

fish1 <- fish %>%
  select(IdOrig,Year,Catch,SpeciesCat,SciName,CommName=Fish_Species) %>%
...
#4/14kei:回帰の比較用
setwd("/Volumes/GoogleDrive/共有ドライブ/gakuLab_Research_Upside/Upside-U.Iwate/Other/GUM-master-Kei/data")
fish <- read.csv("Test.csv",stringsAsFactors = F)

#学名を付与
fish <- fish %>%
  left_join(lifehistory,by=c("Fish_Species"="CommName","SciName","SpeciesCat"))
...
#results = run_gum_assessment_Kei(dat = fish_JP) #RAM
results = run_gum_assessment_Kei(dat = fish1)
