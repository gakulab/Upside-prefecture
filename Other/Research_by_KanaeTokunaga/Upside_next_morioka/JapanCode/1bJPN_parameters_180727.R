...
dt.catch <- read.csv("data/1japan_production.csv")
dt.price <- read.csv("data/1japan_price.csv")
#lumped <- read.csv("C:/Users/kkleisner/Documents/ContractWork/EDF/Upside/Cuba/Feb2018_Romina/data/1ProjectionData.csv", header = T, stringsAsFactors = F)
#fishery.list <- read.csv("miscData/fisheryList.csv")
...
## Run GUM assessment
japan_results <- run_gum_assessmentKK(japan_catch3) ## If this doesn't run, try restart R
...
write.csv(japan.fishery, file = "data/0Upside_inputs_prep_180727.csv")
...
write.csv(Japan_results100, "data/2japan_GUM_results_K100_0727.csv", row.names = FALSE)
...
japan_inputs <- read.csv('data/3Upside_Input_0530.csv',stringsAsFactors=FALSE) #v3 is fine for this one.
...
setwd("~/Dropbox/EDFUPSIDE2018/JapanUpside_2018")
#write.csv(japan_inputs5, "data/4Inputs_for_proj_disc5per_0727.csv", row.names = FALSE)
