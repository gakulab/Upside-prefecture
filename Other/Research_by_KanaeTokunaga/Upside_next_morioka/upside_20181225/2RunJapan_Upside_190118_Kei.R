### Upside理解 ###
## 最終章：政策シミュレーション
## 1bJPN_parameters_mor_kt_kei_git.Rで生物モデルによるアウトプットを作成。本スクリプトでは政策シミュレーションが行われているはず
## アウトプットはmasterPsubであり、ここに辿り着くことを目標とする
## 編集履歴
## 21/3/4-
...
## Enter whether or not to loop over catch share cost and price scalars - put "yes" or "no" #直接関与するのはprojectionModel関数内である
catchShareLoop = "no"

## Enter whether or not to loop over eliminating illegal fishing - put "yes" or "no" #直接関与するのはprojectionModel関数内である
illegalLoop = "no"
