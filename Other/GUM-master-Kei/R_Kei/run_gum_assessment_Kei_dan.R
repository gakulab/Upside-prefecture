...
run_gum_assessment_Kei_dan <- function(dat){
  ...
  #run_post_prm_pt_cmsy_Kei()を使用。独自のオプションPlot(r,kのプロットを表示する)をTRUEにしている。
  apply_fun <- function(i,data,stocks){
    out = run_post_prm_pt_cmsy_Kei_dan(dat = filter(data, IdOrig == stocks[i]),Display = T)$CatchMSY
     #out = run_post_prm_pt_cmsy_Kei(dat = filter(data, IdOrig == stocks[i]),Display = T,Plot = TRUE)
  }
  ...
  results <- lapply(1:length(stocks), apply_fun, data = data, stocks = stocks) %>%
    bind_rows()
  #一つ下の階層には魚種別の総合結果。さらに一つ下の階層には3種の結果(Catch-MSY,Biomass,graph)が含まれる
  #results <- lapply(1:length(stocks), apply_fun, data = data, stocks = stocks)
