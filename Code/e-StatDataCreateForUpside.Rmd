...
#市町村別データ　魚種別漁獲量　全国
CatchData_Species_City <- estat_getStatsData(appId, statsDataId = "0003238629")
...
## 産出額
```{r}
#大海区都道府県別　海面漁業・養殖業産出額、主要魚種別海面漁業産出額
PriceData_Species_2019 <- estat_getStatsData(appId, statsDataId = "0001876613")

#大海区都道府県別　主要魚種別海面養殖業産出額
PriceData_AquaCulture_Species_2019 <- estat_getStatsData(appId, statsDataId = "0001876614")
...
#手順
#1) 半角・全角などの文字の混乱を調整
#2) データ結合時の共通鍵となる県名・県コードの対照表を作成
#3) 結合される側(近年のデータ)のデータを結合する側(長期累計)のデータの形に合うように整える
#4) データを結合する(rbind)


#混乱ポイント
#半角・全角が入り混じっている
#area_codeの全国を示す値が長期累計は00000、2017年以降は00001である

## 魚種別

# 長期累計データの整形
CatchData_Species_all <- CatchData_Species %>%
  mutate(year = as.numeric(str_sub(.$年次,end = -2)), #"年"を除く
         #area_code = ifelse(都道府県=="全国","00001",area_code),
         `漁業魚種（長期累年）` = ifelse(`漁業魚種（長期累年）`=="計","合計",`漁業魚種（長期累年）`),
         `漁業魚種（長期累年）` = ifelse(`漁業魚種（長期累年）`=="魚類_さけ･ます類_小計","魚類_さけ・ます類_小計",`漁業魚種（長期累年）`), #半角と全角の"・"を調整
...
#長期累年データの都道府県（沿岸県+全国）に合わせるためのリスト作成。area_codeで結合する。area_codeにおいて2017年以降は全国が00000に対し、長期累計は00001である。
#長期累計データの都道府県リスト
PrefList <- unique(CatchData_Species_all[,c("area_code","prefecture_plus")]) #長期累計の県コード・県名を抽出。県が付いているので_plus
#近年データの都道府県リスト
PrefList2 <- unique(CatchData_Species_2017[,c("area_code","(C001-2017-1)都道府県・大海区・支庁")]) #2017年以降は"県"が付かない
...
CatchData_Species_2016_Working <- CatchData_Species_2016 %>%
  mutate(year = 2016
         #area_code = ifelse(`(C001-28-1)都道府県・大海区・支庁`=="全国","00001",area_code)
         ) %>%
  left_join(PrefList3) %>%
