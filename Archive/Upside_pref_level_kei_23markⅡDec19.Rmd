...
前段階情報
単位：
dt.weight -> t     dr.price -> 100万円      dt.comp : harvest1 -> t      dt.price -> $

~~~~  大型Update (2019/12/23)  ~~~~

主な変更点
・TempUserの廃止(作業用ディレクトリを作成しprojectのディレクトリをそこに固定した)
・各チャンクでのsetwd()の廃止(rmarkdownの特徴として各チャンクが独立しており、毎回各設定を新たに行う必要があったが、ディレクトリの固定によってseted()が不必要となった)
...
dt.weight<-read.csv("dataKaimenKenbetsu_modified_1962_2015.csv")
...
region5<-read.csv("translation_prefecture_region.csv")#iranai
...
# 漁獲・生産額割合のみを抽出
dt.past <- dt.comp %>% 
  select(prefecture,alphabet_pref,fish_stock,percentage_h,percentage_p) %>%
  unique()
...
dt.graph2 <- dt.graph %>%
  ...
  mutate(check = ifelse(FMSY>SQ,TRUE,FALSE))

#head()を用いて並べる
dt.graph3 <- head(dt.graph2[dt.graph2$FMSY>dt.graph2$SQ,],1)
dt.graph4 <- if(dt.graph3$alphabet_pref=="hokkaido"){dt.graph3}else{rbind(dt.graph3)}
...
if(Temp_User==1){setwd("~/Google ドライブ/GD_Research_Upside/Upside_project/Data_Working")}else if(Temp_User==2){setwd("~/Dropbox/#_0_gakuGroup/Upside_Kei.K/Output")}else if(Temp_User==3){setwd("C:/Users/aquak/Dropbox/Upside_Kei.K/timeseries/prefecture/harvest")}
...
options(scipen=999)
theme_set(theme_bw())
if(Temp_User==1){setwd("~/Google ドライブ/GD_Research_Upside/Upside_project/Data_Working")}else if
...
if(Temp_User==1){setwd("~/Google ドライブ/Upside_Kei/Upside_project/Current_Working_Data/Datasets")}else if
(Temp_User==2){setwd("~/Dropbox/#_0_gakuGroup/Upside_Kei.K/Output")}else if
(Temp_User==3){setwd("C:/Users/aquak/Dropbox/Upside_Kei.K/timeseries/prefecture/profit")}
...
fish1<-dt.comp1 %>%
  ...
  ungroup() %>%
  select(fish_stock,time,fish_total_harvest,management)

fish2<-unique(fish1)

fish3 <- fish2 %>%
  spread(key=management,value=fish_total_harvest) %>%
  mutate(FMSY_over_SQ = ifelse(FMSY>SQ,TRUE,FALSE))
if(sum(fish3$FMSY_over_SQ)>1){
  fish3 <- fish3 %>%
  filter(FMSY_over_SQ==TRUE) %>%
  filter(time==min(time)) %>%
  select(fish_stock,time)
}else{
  fish3 <- fish3 %>%
  filter(time==min(time)) %>%
  select(fish_stock,time)
...
fish4 <- if(fish3$fish_stock==fish_name_fixed){fish3}else{fish4}
fish5 <- if(fish3$fish_stock==fish4$fish_stock){fish4}else{rbind(fish5,fish3)}
...
#write.csv(fish5,"FMSY_exceed_SQ.csv")
...
for (n in 1:nrow(translation_pref_remove_na)) { 
comp_name=translation_pref_remove_na[n,2]
setwd("~/Google ドライブ/Upside_Kei/Upside_project/Current_Working_Data/Datasets")
