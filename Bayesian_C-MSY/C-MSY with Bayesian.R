...
## Read Data for stock, year=yr, catch=ct, and resilience=res. Expects space delimited file with header yr  ct and years in integer and catch in real with decimal point
  ...
  setwd("/Users/yugakisara/Desktop/R/Catch＿MSY")
...
#################################################################################for(stock in stock_id) {
  ...
  start_k     <- c(max(ct),1000*max(ct)) ## default for upper k e.g. 100 * max catch
  ...
  sigR        <- 0.05      ## process error; 0 if deterministic model; 0.05 reasonable value? 0.2 is too high：プロセスエラーの適正値の設定については別途議論されるだろう
  ...
  if(length(r1)>=10) {
    ...
    #B
    plot(r1, k1, xlim = start_r, ylim = start_k, xlab="r1", ylab="k2 (1000t)")
