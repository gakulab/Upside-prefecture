...
・2019年の確報が出たので、2019年の漁獲量データを用いて結果を算出
・
...
load("UpsideDataForResearch3.rda")
...
lapply(1:length(unique(UpsideResult$prefecture)), function(x){
  ...
  #2020年（シミュレーション１年目）のSQ（現状維持政策）におけるtotal_pref_catch（合計漁獲量：t）を基準値として抽出。
  standard <- as.numeric(na.omit(UpsideResult[UpsideResult$prefecture==unique(UpsideResult$prefecture)[x] & UpsideResult$management == "SQ" & UpsideResult$year == 2020,])[,5]) 
...
　###  NPVの期間内合計値を見てみる  ###
　dt.npv <- dt.ere %>%
  　select(prefecture,alphabet,管理シナリオ=management,TotalPrefNPV) %>%
  　unique()
...
ggdata <- UpsideResult %>%
  filter(
    prefecture == "岩手",
    management %in% c("FMSY","SQ","Historical")) %>%
  group_by(year,management) %>%
  summarise(年間漁獲量_10000t = sum(total_pref_catch,na.rm = T)/10000,
            管理施策 = ifelse(management == "FMSY","MSY漁獲政策",
                                ifelse(management == "SQ","現状維持政策","過去漁獲量")),
            年 = year) %>%
  ungroup() %>%
  na.omit() %>%
  unique()

ggdata2 <- japan_catch1 %>%
  group_by(Year) %>%
  summarise(sum = sum(Catch,na.rm = T)/10000) %>%
  ungroup()

ggdata3 <- dt.catch %>%
  group_by(year) %>%
  summarise(sum = sum(catch_t,na.rm = T)/10000) %>%
  ungroup()
...
ggplot(ggdata,aes(x=年,y=年間漁獲量_10000t,col=管理施策,fill=管理施策))+
  theme_bw()+
  ...
  geom_line(size = 1.5)+
  scale_x_continuous(breaks = c(1956,1978,2000,2020,2045,2069)) +
  scale_color_manual(values=c("MSY漁獲政策"="#35a16b","現状維持政策"="#ff2800","過去漁獲量"="grey"))+
  labs(title = "岩手県における時系列漁獲量と予測漁獲量(88魚種・系群)")
