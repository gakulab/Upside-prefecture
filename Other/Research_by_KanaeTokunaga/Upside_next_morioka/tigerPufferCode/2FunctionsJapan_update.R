...
## Further Updated to accomodate up to 4 fleets by Kanae Tokunaga (Sep 19, 2018)
################################################
...
bioModel = function(b,phi,g,f_intervention1,f_intervention2,f_intervention3,f_intervention4,f_nonintervention)
{
  b_next = b + ((phi + 1) / phi ) * g * b * (1 -  (b ^ phi)  / (phi + 1)) - g * f_intervention1 * b - g * f_intervention2 * b - g * f_intervention3 * b - g * f_intervention4 * b - g * f_nonintervention * b
...
profitOptim = function(f_intervention, f_nonintervention, b, p1, p2, p3, p4,  K, c1, c2, c3, c4, g, phi, beta, V, bvec, delta, split1, split2, split3)
{  
  ...
  f_intervention1 = split1 * f_intervention
  f_intervention2 = split2 * f_intervention
  f_intervention3 = split3 * f_intervention
  f_intervention4 = (1 - split1 - split2 - split3)* f_intervention
  
  profit1 = econModel(g ,K, phi, p1, f_intervention1, b, c1, beta)$pi
  profit2 = econModel(g, K, phi, p2, f_intervention2, b, c2, beta)$pi
  profit3 = econModel(g, K, phi, p3, f_intervention3, b, c3, beta)$pi
  profit4 = econModel(g, K, phi, p4, f_intervention4, b, c4, beta)$pi
  
  profit = profit1 + profit2 + profit3 + profit4
    
  bnext = bioModel(b, phi, g, f_intervention1, f_intervention2,  f_intervention3, f_intervention4, f_nonintervention)
...
dynamicPolicy = function(K,g,phi,p1,p2,p3, p4,c1,c2,c3,c4,beta,disc,bvec,f_nonintervention,split1,split2,split3)
{
  ...
  while (t<4 | diff>tol)
  {
    ...
    for (i in 1:length(bvec))
    {
      ...
      FishOut= optim(par=guess,fn=profitOptim,gr=NULL,lower=0,upper=((phi+1)/phi-0.01-f_nonintervention), f_nonintervention=f_nonintervention,b=b,p1=p1, p2=p2, p3=p3, p4=p4, K=K,c1=c1, c2=c2, c3=c3, c4=c4,g=g,phi=phi,beta=beta,V=V,bvec=bvec,delta=delta,split1=split1, split2=split2, split3=split3,method="L-BFGS-B")
...
policy = function(s,g,K,phi,p1,p2,p3, p4, f0Int1,f0Int2,f0Int3, f0Int4, f0NonInt,c1,c2,c3,c4,beta,disc,bvec,split1,split2,split3)
{

 ## Policy vector for status quo; maintain f0_Intervention
 if (s==1) {f1 = rep(f0Int1,length(bvec))
           f2 = rep(f0Int2,length(bvec))
           f3 = rep(f0Int3,length(bvec))
           f4 = rep(f0Int4,length(bvec))}
 
 ## Policy vector that maximimzes food production; set at f=1 for all conditions, including f_NonIntervention
 ## Split betwen four legal fleets using split
 if (s==2) {f1 = rep(max(0, (1 - f0NonInt) * split1),length(bvec))
           f2 = rep(max(0, (1 - f0NonInt) *split2), length(bvec))
           f3 = rep(max(0, (1 - f0NonInt) *split3), length(bvec))
           f4 = rep(max(0, (1 - f0NonInt) *(1 - split1 - split2 - split3)), length(bvec))}
 
 ## Policy vector that minimizes recovery time; set at 0 until b=1 then set at f=1, including f_NonIntervention
 ## Split between four legal fleets using split
 zeros = which(bvec<1)
 ones = which(bvec>=1)
 if (s==3) {f1 = c(rep(0,length(zeros)),rep(max(0,(1-f0NonInt) * split1),length(ones)))
            f2 = c(rep(0,length(zeros)),rep(max(0,(1-f0NonInt) * split2),length(ones)))
            f3 = c(rep(0,length(zeros)),rep(max(0,(1-f0NonInt) * split3),length(ones)))
            f4 = c(rep(0,length(zeros)),rep(max(0,(1-f0NonInt) * (1 - split1 - split2 - split3)),length(ones)))}

 ## Policy vector that dynamically maximizes NPV
 ## Split between four legal fleets using split
 if (s==4) {f1 = dynamicPolicy(K,g,phi,p1,p2,p3,p4,c1,c2,c3,c4,beta,disc,bvec,f0NonInt,split1,split2,split3)$f1 * split1
            f2 = dynamicPolicy(K,g,phi,p1,p2,p3,p4,c1,c2,c3,c4,beta,disc,bvec,f0NonInt,split1,split2,split3)$f1 * split2
            f3 = dynamicPolicy(K,g,phi,p1,p2,p3,p4,c1,c2,c3,c4,beta,disc,bvec,f0NonInt,split1,split2,split3)$f1 * split3
            f4 = dynamicPolicy(K,g,phi,p1,p2,p3,p4,c1,c2,c3,c4,beta,disc,bvec,f0NonInt,split1,split2,split3)$f1 * (1-split1-split2-split3)}
...
breakEvenNPV = function(discount,P1,P2,P3,P4)
{
  for (i in 1:length(P1))
  {
    npvCurrent1 = NPV(discount,P1[1:i])
    npvCurrent2 = NPV(discount,P2[1:i])
    npvCurrent3 = NPV(discount,P3[1:i])
    npvCurrent4 = NPV(discount,P4[1:i])
    if (npvCurrent1 + npvCurrent2 + npvCurrent3 + npvCurrent4 >= 0) pointer = i-1
    if (npvCurrent1 + npvCurrent2 + npvCurrent3 + npvCurrent4 >= 0) break
    if (i==length(P1) & npvCurrent1 + npvCurrent2 + npvCurrent3 + npvCurrent4 <= 0) pointer=NA
  }
  return(pointer)
}
...
projectionModel = function(params,S,CatchShareLoop,LegalLoop)
{
  ...
  ## initialize policies3 array
  policies3 = array(NA,dim=c(length(S),N,L,M,length(bVEC)))
  
  ## initialize policies4 array
  policies4 = array(NA,dim=c(length(S),N,L,M,length(bVEC)))
  ...
  ## initialize intervention fishing mortality reference point projection array
  fInt3Projections = array(NA,dim=c(length(S),N,L,M,length(delayVec),time))
  
  ## initialize intervention fishing mortality reference point projection array
  fInt4Projections = array(NA,dim=c(length(S),N,L,M,length(delayVec),time))
  ...
  ## initialize intervention absolute harvest projection array
  HInt3Projections = array(NA,dim=c(length(S),N,L,M,length(delayVec),time))
  
  ## initialize intervention absolute harvest projection array
  HInt4Projections = array(NA,dim=c(length(S),N,L,M,length(delayVec),time))
  ...
  ## initialize revenue projection array
  revenue3Projections = array(NA,dim=c(length(S),N,L,M,length(delayVec),time))
  
  ## initialize revenue projection array
  revenue4Projections = array(NA,dim=c(length(S),N,L,M,length(delayVec),time))
  ...
  ## initialize cost projection array
  cost3Projections = array(NA,dim=c(length(S),N,L,M,length(delayVec),time))
  
  ## initialize cost projection array
  cost4Projections = array(NA,dim=c(length(S),N,L,M,length(delayVec),time))
  ...
  ## initialize profit projection array
  profit3Projections = array(NA,dim=c(length(S),N,L,M,length(delayVec),time))
  
  ## initialize profit projection array
  profit4Projections = array(NA,dim=c(length(S),N,L,M,length(delayVec),time))
  ...
  ## initialize NPV matrix
  npv3 = array(NA,dim=c(length(S),N,L,M,length(delayVec)))
  
  ## initialize NPV matrix
  npv4 = array(NA,dim=c(length(S),N,L,M,length(delayVec)))
  ...
  ## initialize annuity matrix
  annuity3 = array(NA,dim=c(length(S),N,L,M,length(delayVec)))
  
  ## initialize annuity matrix
  annuity4 = array(NA,dim=c(length(S),N,L,M,length(delayVec)))
        ...
        # for (k in 1:Thetas){
          ...
          for (l in 1:L){
            ...
            for (m in 1:M){
              ...
              theta3N_legal = params$theta_legal3_expected
              theta4N_legal = params$theta_legal4_expected
              thetaN_domestic = params$theta_domestic
              p1N = params$p1_expected
              p2N = params$p2_expected
              p3N = params$p3_expected
              p4N = params$p4_expected
              betaN = params$beta_expected	
              c1N = params$c1_expected
              c2N = params$c2_expected
              c3N = params$c3_expected
              c4N = params$c4_expected
              gamma_p1N = params$gamma_p1
              gamma_p2N = params$gamma_p2
              gamma_p3N = params$gamma_p3
              gamma_p4N = params$gamma_p4
              gamma_c1N = params$gamma_c1
              gamma_c2N = params$gamma_c2
              gamma_c3N = params$gamma_c3
              gamma_c4N = params$gamma_c4
              lambdaN = params$lambda
              split1N = params$split1
              split2N = params$split2
              split3N = params$split3
              ...
              theta3N_legal = parameterPull(params$theta_legal3_upper,params$theta_legal3_lower,params$theta_legal3_expected)
              theta4N_legal = parameterPull(params$theta_legal4_upper,params$theta_legal4_lower,params$theta_legal4_expected)
              thetaN_domestic = params$theta_domestic
              ...
              p3N = parameterPull(params$p3_upper,params$p3_lower,params$p3_expected)
              p4N = parameterPull(params$p4_upper,params$p4_lower,params$p4_expected)
              ...
              c3N = parameterPull(params$c3_upper,params$c3_lower,params$c3_expected)
              c4N = parameterPull(params$c4_upper,params$c4_lower,params$c4_expected)
              gamma_p1N = params$gamma_p1
              gamma_p2N = params$gamma_p2
              gamma_p3N = params$gamma_p3
              gamma_p4N = params$gamma_p4
            ...
            f0Int3N = f0_totalN * theta3N_legal * thetaN_domestic
            
            f0Int4N = f0_totalN * theta4N_legal * thetaN_domestic
            
            f0NonIntN = f0_totalN * (1 - (theta1N_legal * thetaN_domestic) - (theta2N_legal * thetaN_domestic) - (theta3N_legal * thetaN_domestic) - (theta4N_legal * thetaN_domestic))
            
            policies[i,j,l,m,] = policy(S[i],gN,KN,phiN,p1N,p2N,p3N, p4N, f0Int1N,f0Int2N,f0Int3N,f0Int4N,f0NonIntN,c1N,c2N,c3N,c4N,betaN,discN,bVEC,split1N, split2N, split3N)$f1 
            policies2[i,j,l,m,] = policy(S[i],gN,KN,phiN,p1N,p2N,p3N, p4N, f0Int1N,f0Int2N,f0Int3N,f0Int4N,f0NonIntN,c1N,c2N,c3N,c4N,betaN,discN,bVEC,split1N, split2N, split3N)$f2
            policies3[i,j,l,m,] = policy(S[i],gN,KN,phiN,p1N,p2N,p3N, p4N, f0Int1N,f0Int2N,f0Int3N,f0Int4N,f0NonIntN,c1N,c2N,c3N,c4N,betaN,discN,bVEC,split1N, split2N, split3N)$f3 
            policies4[i,j,l,m,] = policy(S[i],gN,KN,phiN,p1N,p2N,p3N, p4N, f0Int1N,f0Int2N,f0Int3N,f0Int4N,f0NonIntN,c1N,c2N,c3N,c4N,betaN,discN,bVEC,split1N, split2N, split3N)$f4
            
            ## Set non-intervention f0 depending on illegal harvest elimination loop
            if (m == 1) f0NonIntN = f0_totalN * (1 - (theta1N_legal * thetaN_domestic) - (theta2N_legal * thetaN_domestic) - (theta3N_legal * thetaN_domestic)  - (theta4N_legal * thetaN_domestic)) 
            ...
            for (n in 1:time){
              ...
              #print(paste("Scenario ",i,"of",length(S),"; Monte Carlo Iteration",j,"of",N,"; Theta", k,"of",Thetas,"; Catch Share Loop",l,"of",L,"; Illegal Elimination Loop",m,"of",M,"; Time Step",n,"of",T,sep=" "))
                ...
                if (n == 1) {
                    ...
                    fInt3Projections[i,j,l,m,h,n] = f0Int3N
                    fInt4Projections[i,j,l,m,h,n] = f0Int4N
                    ...
                    if (l == 2 & n >= delayVec[h]) {  ## added code here
                        ...
                        p3N = params$p3_expected * gamma_p3N
                        p4N = params$p4_expected * gamma_p4N
                        c1N = params$c1_expected * gamma_c1N
                        c2N = params$c2_expected * gamma_c2N
                        c3N = params$c3_expected * gamma_c3N
                        c4N = params$c4_expected * gamma_c4N
                    }
                    ...
                    if (S[i] == 6){
                        if (n >= delayVec[h]) { # Only implement policy if implementation year is reached
                            ...
                            fInt3Projections[i,j,l,m,h,n] = max(0,min(maxFN,fInt3Projections[i,j,l,m,h,n-1] + 
                                                                        lambdaN * 
                                                                        profit3Projections[i,j,l,m,h,n-1] / profitMSY(gN,KN,phiN,p3N,bProjections[i,j,l,m,h,n-1],c3N,betaN)))
                            
                            fInt4Projections[i,j,l,m,h,n] = max(0,min(maxFN,fInt4Projections[i,j,l,m,h,n-1] + 
                                                                        lambdaN * 
                                                                        profit4Projections[i,j,l,m,h,n-1] / profitMSY(gN,KN,phiN,p4N,bProjections[i,j,l,m,h,n-1],c4N,betaN)))
                        }
                     else {
                      fInt1Projections[i,j,l,m,h,n] = f0Int1N
                      fInt2Projections[i,j,l,m,h,n] = f0Int2N
                    ...
                    if (S[i] < 6) {
                        if (n >= delayVec[h]) { # Only implement improved policy if implementation year is reached
                            if (bProjections[i,j,l,m,h,n] < bVEC[1]) {
                               ...
                               fInt3Projections[i,j,l,m,h,n] = approx(bVEC,policies3[i,j,l,m,],bVEC[1])$y
                               fInt4Projections[i,j,l,m,h,n] = approx(bVEC,policies4[i,j,l,m,],bVEC[1])$y 
                                ...
                                fInt3Projections[i,j,l,m,h,n] = approx(bVEC,policies3[i,j,l,m,],bProjections[i,j,l,m,h,n])$y
                                fInt4Projections[i,j,l,m,h,n] = approx(bVEC,policies4[i,j,l,m,],bProjections[i,j,l,m,h,n])$y
                            }
                         } else {
                          fInt1Projections[i,j,l,m,h,n] = f0Int1N
                          fInt2Projections[i,j,l,m,h,n] = f0Int2N
              ...
              fTotalProjections[i,j,l,m,h,n] = fInt1Projections[i,j,l,m,h,n] + fInt2Projections[i,j,l,m,h,n] + fInt3Projections[i,j,l,m,h,n] + fInt4Projections[i,j,l,m,h,n] + fNonIntProjections[i,j,l,m,h,n]
              ...
              HInt3Projections[i,j,l,m,h,n] = fInt3Projections[i,j,l,m,h,n] * gN * bProjections[i,j,l,m,h,n] * BMSY(KN,phiN)
              
              HInt4Projections[i,j,l,m,h,n] = fInt4Projections[i,j,l,m,h,n] * gN * bProjections[i,j,l,m,h,n] * BMSY(KN,phiN)
              
              HNonIntProjections[i,j,l,m,h,n] = fNonIntProjections[i,j,l,m,h,n] * gN * bProjections[i,j,l,m,h,n] * BMSY(KN,phiN)
              
              HTotalProjections[i,j,l,m,h,n] = HInt1Projections[i,j,l,m,h,n] + HInt2Projections[i,j,l,m,h,n] + HInt3Projections[i,j,l,m,h,n] + HInt4Projections[i,j,l,m,h,n] +HNonIntProjections[i,j,l,m,h,n]
              ...
              revenue3Projections[i,j,l,m,h,n] = econModel(gN,KN,phiN,p3N,fInt3Projections[i,j,l,m,h,n],bProjections[i,j,l,m,h,n],c3N,betaN)$revenue
              
              revenue4Projections[i,j,l,m,h,n] = econModel(gN,KN,phiN,p4N,fInt4Projections[i,j,l,m,h,n],bProjections[i,j,l,m,h,n],c4N,betaN)$revenue
              ...
              cost3Projections[i,j,l,m,h,n] = econModel(gN,KN,phiN,p3N,fInt3Projections[i,j,l,m,h,n],bProjections[i,j,l,m,h,n],c3N,betaN)$cost
              
              cost4Projections[i,j,l,m,h,n] = econModel(gN,KN,phiN,p4N,fInt4Projections[i,j,l,m,h,n],bProjections[i,j,l,m,h,n],c4N,betaN)$cost
              ...
              profit3Projections[i,j,l,m,h,n] = econModel(gN,KN,phiN,p3N,fInt2Projections[i,j,l,m,h,n],bProjections[i,j,l,m,h,n],c3N,betaN)$pi
              
              profit4Projections[i,j,l,m,h,n] = econModel(gN,KN,phiN,p4N,fInt4Projections[i,j,l,m,h,n],bProjections[i,j,l,m,h,n],c4N,betaN)$pi
              
              if (n < time) bProjections[i,j,l,m,h,n+1] = bioModel(bProjections[i,j,l,m,h,n],phiN,gN,fInt1Projections[i,j,l,m,h,n],fInt2Projections[i,j,l,m,h,n],fInt3Projections[i,j,l,m,h,n],fInt4Projections[i,j,l,m,h,n],fNonIntProjections[i,j,l,m,h,n])
            ...
            npv3[i,j,l,m,h] = NPV(discN,profit3Projections[i,j,l,m,h,])
            
            npv4[i,j,l,m,h] = NPV(discN,profit4Projections[i,j,l,m,h,])
            ...
            annuity3[i,j,l,m,h] = annuityFunc(discN,profit3Projections[i,j,l,m,h,])
            
            annuity4[i,j,l,m,h] = annuityFunc(discN,profit4Projections[i,j,l,m,h,])
            
            breakEven[i,j,l,m,h] = breakEvenNPV(discN,profit1Projections[i,j,l,m,h,],profit2Projections[i,j,l,m,h,],profit3Projections[i,j,l,m,h,],profit4Projections[i,j,l,m,h,])
  ...
  return(list(bVEC=bVEC,
              ...
              fInt3Projections=fInt3Projections,
              fInt4Projections=fInt4Projections,
              ...
              HInt3Projections=HInt3Projections,
              HInt4Projections=HInt4Projections,
              ...
              revenue3Projections=revenue3Projections,
              revenue4Projections=revenue4Projections,
              cost1Projections=cost1Projections,
              cost2Projections=cost2Projections,
              cost3Projections=cost3Projections,
              cost4Projections=cost4Projections,
              profit1Projections=profit1Projections,
              profit2Projections=profit2Projections,
              profit3Projections=profit3Projections,
              profit4Projections=profit4Projections,
