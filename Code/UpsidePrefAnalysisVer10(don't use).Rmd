...
・Upsideデータの都道府県への分配方式をTAC設定に基づき、過去３カ年の漁獲実績に変更しました。
・ディレクトリを変更しました。（githubを見越して、ローカルのパスに変更）
...
base <- ggplot(b1,aes(x = year, y = total_pref_catch, color = Management)) +
  ...
  geom_line(size=.5)+
  geom_point(aes(shape = Management),size=1.5)+
  annotate("rect", xmin=2016,xmax=2065, ymin=8,ymax=13.5,
             fill="#a9a9a9",alpha = .3)
...
setwd("/Volumes/GoogleDrive/Shared drives/gakuLab_Research_Upside/Upside-prefecture/Outputs/UpsidePrefAnalysisOutput/<English> 時系列とシミュレーション結果の混合")
ggsave(file = "IwateTimeseries2.png",plot = iwate,dpi = 350, width = 4.8, height = 5.5)
...
base <- ggplot(b1,aes(x = year, y = total_pref_catch, color = Management)) +
  ...
  geom_line(size=.5)+
  geom_point(aes(shape = Management),size=1.5)+
  annotate("rect", xmin=2016,xmax=2065, ymin=13,ymax=21,
             fill="#a9a9a9",alpha = .3)
...
add2 <- ggplotGrob(add)
miyagi <- base + annotation_custom(add2,xmin = 2012,xmax = 2068,ymin = 22,ymax = 62)

miyagi

setwd("/Volumes/GoogleDrive/Shared drives/gakuLab_Research_Upside/Upside-prefecture/Outputs/UpsidePrefAnalysisOutput/<English> 時系列とシミュレーション結果の混合")
ggsave(file = "MiyagiTimeseries2.png",plot = miyagi,dpi = 350, width = 4.8, height = 5.5)
...
　#移動平均でfacetを使うため、県は手動で分ける
  ...
  　     x = "Time",
    ...
    theme(
        ...
        plot.title = element_text(hjust = 0.5,face = "bold",size = 20),
        plot.subtitle = element_text(hjust = 0.5)) +
  　scale_fill_manual(values=c("FMSY"="#35a16b","現状維持政策"="#ff2800"))
...
#岩手県の場合：54魚種
unique(filter(UpsideResultDetail,alphabet=="iwate",pref_catch > 0,year==2016)$fish_stock)


#宮城県の場合：55魚種
unique(filter(UpsideResultDetail,alphabet=="miyagi",pref_catch > 0,year == 2016)$fish_stock)
