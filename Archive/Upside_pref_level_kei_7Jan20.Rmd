...
geom_kobe(data=all_biomass,years = "2020")
...
Temp_Data_Catch2 <- Temp_Data_Catch1 %>%
  group_by(year,management) %>%
  mutate(total_harvest=sum(harvest1)) %>% #その年の総漁獲量
  mutate(total_profit=sum(profit1)) %>% #その年の総利益
  ungroup()

dt.ts <- left_join(dt.weight2,dt.price2,by=c("year","management"))
dt.ts <- merge(dt.ts,Temp_Data_Catch2,all=T)
dt.ts$total_harvest_mt <- dt.ts$total_harvest/1000000 #単位を100万tに変更
dt.ts$management<-factor(dt.ts$management,levels = c("FMSY","SQ","econOpt","Historical"))

#プロット
g1 <- ggplot(dt.ts,aes(x=year,y=total_harvest_mt,color=management,shape=management)) +
  geom_point() +
