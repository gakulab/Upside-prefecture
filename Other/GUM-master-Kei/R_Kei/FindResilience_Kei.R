...
　　
  ...
  for(a in 1:length(cats))
  {
    ...
    temp<-ResCount[ResCount$SpeciesCatName==cats[a],] #tempにSpeciesCatNameのもつ全種のレジリエンスを格納

    DefaultRes$Res[a]<-temp$Res[temp$Count==max(temp$Count,na.rm=T)] #SpeciesCatNameのもつレジリエンスの中で最多のレジリエンスを付与する。同数の場合は勝手にどちらかが採用される。
  }
