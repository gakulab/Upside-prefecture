...
#save(list = c("dt.comp4","dt.comp2","dt.c_and_p","dt.c_and_p3"),file = "Upside_pref.rda")
#load("Upside_pref.rda")
```
...
for (i in 1:39) {
  ...
  #基準となるSQの値を抽出
  sTaRt <- dt.comp4 %>%
    filter(prefecture == pref_list[i], year==2016, management == "SQ")
  sTaRt <- as.numeric(sTaRt[1,5])
  ...
  #dt.graph2 <- dt.graph %>%
  #  mutate(ratio = total_pref_catch/sTaRt) %>%
  #  merge(brank,all = T)
  ...
  plot(g3)
  setwd("/Volumes/GoogleDrive/Shared drives/gakuLab_Research_Upside/Upside-U.Iwate/Other/Processing_Industry/知事表彰2020/Plot3")
